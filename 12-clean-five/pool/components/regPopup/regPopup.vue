<!-- 注册组件 -->
<template>
	<div class="reg-popup fadeIn"  ref="regPopup" @click="close" style="animation-duration: 0.3s;">
		<div class="contentWrap" ref="contentWrap" @click.stop="hideDropdown($event)">
			<div class="headerTitle">
				<div class="title">{{option.typeText}}</div>
				<div class="rightBox">
					<div class="callout" @click="toggleType()">{{option.toggleText}}</div>
					<div class="toggleTypeIcon" @click="toggleType()">
						<img v-show="option.type != 'accountLogIn'" src="~assets/images/register/pc.png" class="pcIcon" alt="">
						<img v-show="option.type == 'accountLogIn'" src="~assets/images/register/phone.png" class="phoneIcon" alt="">
					</div>
				</div>
			</div>
				...
				<div class="formList">
					...
					<div v-show="option.type == 'signIn'">
						<div class="inputBox">
							...
							<input type="text" v-model.trim="signInOption.mail" @blur="mailBlur($event)" @focus="inputFocus" placeholder="请输入电子邮箱地址">
						</div>
						...
						<div class="passwordBox">
							...
							<input type="password" v-model.trim="signInOption.passwordOne"  @blur="passwordBlur($event, 1)" @input="detectCapsLock($event)" @focus="passwordFocus" autocomplete="new-password" placeholder="请输入登陆密码" maxlength="18">
							<i class="icon iconfont icon-xianshi" @click="changeInputType($event)"></i>
						</div>
						<div class="textMsgBox">
						...
						<div class="passwordBox">
							...
							<input type="password" v-model.trim="signInOption.passwordTwo" @blur="passwordBlur($event, 2)" @input="detectCapsLock($event)" @focus="passwordFocus" @keyup.enter="signInEvent" autocomplete="new-password" placeholder="请再次输入登陆密码" maxlength="18">
							<i class="icon iconfont icon-xianshi" @click="changeInputType($event)"></i>
						</div>
						<div class="textMsgBox">
							<p class="textMsg">两次输入的内容不符，请重新检查</p>
						</div>
						<div class="goBtn" @click="signInEvent">注册</div>
					</div>
					<!-- 账号登录 -->
					<div v-show="option.type == 'accountLogIn'">
						...
						<div class="passwordLoginBox">
							<div class="inputBox">
								...
								<input type="text" v-model.trim="accountLoginOption.account" @blur="accountBlur($event)" @focus="inputFocus"  placeholder="邮箱/用户名/已验证手机号">
							</div>
							<div class="textMsgBox">
								<p class="textMsg">输入格式错误，请重新检查</p>
							</div>
							<div class="inputBox">
								...
								<input type="password" v-model.trim="accountLoginOption.password" @blur="passwordLoginBlur($event)" @input="detectCapsLock($event)" @focus="inputFocus" maxlength="18"  @keyup.enter="passwordLoginEvent" autocomplete="new-password"  placeholder="请输入密码">
								<div class="forgetBox" @click="forgetPasswordEvent"><i class="iconfont icon-wenhao"></i>&nbsp;忘记密码</div>
							</div>
							<div class="textMsgBox">
								<p class="textMsg">输入格式错误，请重新检查</p>
							</div>
							<div class="goBtn" @click="passwordLoginEvent">登录</div>
					...
					<div v-show="option.type == 'codeLogIn'">
						<div>
							<div class="inputBox">
								<div class="label">手机号码</div>
								<input type="number" v-model.trim="codeLoginOption.phone" pattern="[0-9]*" @blur="phoneBlur($event)" @focus="inputFocus" placeholder="请输入手机号码">
								<!-- <vue-tel-input 
							...
							<div class="getCodeBox">
								<div class="inputBox shortBox">
									...
									<input type="text" v-model.trim="codeLoginOption.code" oninput="if(value.length>6)value=value.slice(0,6)" pattern="[0-9]*" @blur="codeBlur($event)" @focus="codeFocus" @keyup.enter="codeLoginEvent" placeholder="请输入验证码">
								</div>
								<button class="getCodeBtn" ref="codeLoginGetCode" @click="codeLoginGetCode">获取验证码</button>
							</div>
							<div class="textMsgBox">
								<p class="textMsg">验证码错误，请重新检查</p>
							</div>
							<div class="goBtn" @click="codeLoginEvent">登录</div>
							<div class="quickRegister"><span @click="option.type = 'signIn'">快速注册</span></div>
						</div>
					</div>
					<!-- 找回密码 -->
					<div v-show="option.type == 'backTo'">
						<div>
							<div class="inputBox">
								<div class="label">账号</div>
								<input type="text"  v-model.trim="backToOption.account" @blur="accountBlur($event)" @focus="inputFocus" placeholder="邮箱/已验证手机号">
							</div>
							<div class="textMsgBox">
							...
							<div class="getCodeBox">
								<div class="inputBox shortBox">
									<div class="label">验证码</div>
									<input type="text" v-model.trim="backToOption.code" oninput="if(value.length>6)value=value.slice(0,6)" pattern="[0-9]*" @blur="codeBlur($event)" @focus="codeFocus"  placeholder="请输入收到的验证码">
								</div>
								<button class="getCodeBtn" ref="backToGetCode" @click="backToGetCode">获取验证码</button>
							</div>
							<div class="textMsgBox">
								<p class="textMsg">验证码错误，请重新检查</p>
							</div>
							<div class="inputBox">
								...
								<input type="password" v-model.trim="backToOption.newPassword" @blur="backToPasswordBlur($event)" @input="detectCapsLock($event)" @focus="inputFocus" maxlength="18" @keyup.enter="backToEvent" autocomplete="new-password" placeholder="请输入登陆密码，至少6位字符">
							</div>
							<div class="textMsgBox">
								<p class="textMsg">6~18位字符，只能包含英文字母、数字、下划线</p>
							</div>
							<div class="goBtn" @click="backToEvent">确定</div>
	...
	export default {
		...
		methods: {
			...
			*/
			...
			clearInputValue() {
				for (const key in this.signInOption) {
					this.signInOption[key] = '';
				}
				for (const key in this.codeLoginOption) {
					this.codeLoginOption[key] = '';
				}
				for (const key in this.accountLoginOption) {
					this.accountLoginOption[key] = '';
				}
				for (const key in this.backToOption) {
					this.backToOption[key] = '';
				}
				let msgNodes = this.$refs.contentWrap.getElementsByClassName('textMsg');
			...
			signInEvent() { // 注册
				...
				if(!this.signInOption.mail || !this.signInOption.phone || !this.signInOption.passwordOne || !this.signInOption.passwordTwo) {
					this.$toast('请完善注册信息！');
					return
				}
			...
			codeLoginGetCode() { // 获取验证码(手机验证码登录)
				if(this.$common.isPhoneNumber(this.codeLoginOption.phone)) { // 合法手机号
					...
					this.$api.sendPhoneCode(data).then(res => { // 获取验证码
						let element = this.$refs.codeLoginGetCode;
			...
			backToGetCode() {
				...
				if(this.backToOption.type == 'phone') { // 发送手机验证码
					...
					this.$api.sendPhoneCode(data).then(res => { // 获取验证码
						let element = this.$refs.backToGetCode;
