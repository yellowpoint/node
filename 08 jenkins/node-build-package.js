...
if (home && home.length>4) {
	...
	// 由于uni项目在本机构建，替换前缀的js文件不确定，故手动匹配指定目录下的index开头的js
	if(project.pack=='uni'&&reources.length==0){
		let tddFile = './unpackage/dist/build/h5/static/js'
		let files = fs.readdirSync(tddFile);
		files.forEach(file => {
			if (/^index/.test(file)) {
				reources.push(tddFile+'/'+file)
				console.log('淘多多替换前缀的js文件',reources);
			}
		});
	}
