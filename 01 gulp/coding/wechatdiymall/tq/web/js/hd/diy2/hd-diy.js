...
var activeAreaWidth = $('.activeArea').width(), //如果为空或设置其他值会报错
	...
	maxAdditive = 10,
	//来自团购
	fromgroup = GetQueryString('fromgroup') || false;
...
function getGoodsData() {
	...
	var dataBox = '',
		...
		successFunc = function(data) {
			...
			//来自团购 去掉反馈机型链接 以及修改为团购价格，还有跳转订单页面url添加fromgroup=yes
			if(fromgroup) {
				$('.feedback').remove()
			}
	...
	$('body').off('click', '.imgModule .moduleMain .mainItem.picture .moduleOption').on('click', '.imgModule .moduleMain .mainItem.picture .moduleOption', function() {
		...
		image.onload = function() {
			...
			function continueAdd2() {
				...
				//2017.10.20 图片添加后的默认大小要铺满超出一部分，故将宽度或者高度设置为110%;由于传给后台的数据是直接取的最后的大小，故图片的大小数据不需要乘以1.1;
				imgWidth = activeAreaWidth;
				imgHeight = imgWidth / imgRealWidth * imgRealHeight;
...
function sendData(url, sendSuccessFun, sendErrorFun) {
	...
	additiveData.groupUserId = GetQueryString('groupUserId')||''; //团购用户id
		...
		function sendUserData() {
			...
			var sendSuccessFun = function(data) {
				...
				if(data.code == 200) {
					...
					//来自拼团则把该信息传给下一个页面
					if(fromgroup) {
						window.location.href = 'orderConfirm.html?channel=' + channel + '&fromgroup='+ fromgroup +'&tradeOrderId=' + data.data.id;
